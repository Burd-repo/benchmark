...
- Session reconnects may reuse an unexpired verification when identity, key, hardware, GPU and runtime remain unchanged. Key rotation/revocation, hardware or runtime drift, stale observations, proof-image policy changes and verification expiry deny admission.
- The proof image is owned by `BURD_CONTROL_RUNTIME_PROOF_IMAGE_REF`; challenge issuance cannot select another digest.
- Windows remains denied until its physical multi-GPU WSL2/NVIDIA isolation gate passes. Runtime admission is not yet consumed by the scheduler, marketplace or production Provider Job Worker.
