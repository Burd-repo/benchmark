...
- The control plane can create backend-authorized compute jobs for a specific provider/device/session, let an authenticated provider session pull the next job, issue job-scoped data-plane grants, revalidate lease and Runtime Admission authority again at acceptance, record progress events, accept final result metadata, and cancel non-terminal jobs.
...
- Provider sessions pull a job over authenticated outbound session credentials and receive a complete backend-validated bundle containing the job, offered lease, data-plane grant, and execution specification. The specification binds provider/device/session, GPU UUID, approved template, digest-pinned image, timeout, expiries, and restrictive runtime policy without copying the raw credential. Acceptance locks the assigned job and latest lease, requires a current exact binding and unexpired offer, and re-evaluates Runtime Admission before execution may begin; denial requeues the job, invalidates its credential, terminalizes active lease authority, and audits the reason.
...
- Lease status follows `offered -> accepted -> provisioning -> active -> completed | failed | expired`. Acceptance updates exactly one current offered lease in the same transaction as the job, after lease binding/expiry and Runtime Admission revalidation; zero-row acknowledgement fails closed.
