...
- Data-plane credentials remain in the Agent. Per-job workspaces are private, downloads/uploads are bounded and SHA-256 verified while streaming, output finalization is atomic, and the container receives files only through an anonymous Docker volume plus bounded output tmpfs and structured `docker cp`; no host bind path or credential enters the container.
