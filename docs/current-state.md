...
- Production `remote-session connect` intentionally does not enable provider jobs. Disconnected Linux-native and Windows/WSL2 Docker/NVIDIA executor backends, the provider artifact byte plane, runtime verification, derived runtime admission, scheduler/assignment/acceptance admission, active-job cancellation discovery, bounded artifact HTTP phases, and the reproducible physical NVIDIA gate harness now exist. Accepted physical NVIDIA evidence, customer input ingestion, external object-storage signing/adapters, WebSocket cancellation push, controlled activation, paid execution, multi-GPU jobs, and multi-provider jobs remain incomplete. Scheduler leases and later metering/marketplace registries remain separate backend capabilities.
