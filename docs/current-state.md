...
- Production `remote-session connect` intentionally does not enable provider jobs. Disconnected Linux-native and Windows/WSL2 Docker/NVIDIA executor backends, the provider artifact byte plane, runtime verification, derived runtime admission, scheduler/assignment/acceptance admission and active-job cancellation discovery now exist, but customer input ingestion, external object-storage signing/adapters, physical Windows GPU-isolation verification, WebSocket cancellation push, controlled activation, paid execution, multi-GPU jobs, and multi-provider jobs remain unimplemented. Scheduler leases and later metering/marketplace registries remain separate backend capabilities.
