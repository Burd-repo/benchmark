...
- BN-12 does not implement automatic WSL2/Docker/driver setup, job submission, customer artifact ingress, arbitrary shell execution, production runtime verification, active-job remote cancellation discovery, paid execution, Kubernetes, or distributed workloads.
...
- The control plane exposes the job lifecycle endpoints plus job-credential-protected artifact download and result upload endpoints. OpenAPI includes the JSON and binary streaming contracts.
