...
- `burd-agent` now contains a production-shaped but disconnected `DockerNvidiaProviderJobExecutor`, a runtime-backend interface, and `LinuxNativeDockerBackend`. It uses structured Docker CLI arguments without a shell, exact template/digest authorization, leased GPU UUID binding, hardened container flags, bounded/redacted logs, typed exit/OOM handling, wall-clock/cancellation deadlines for every Docker/NVIDIA child, explicit `TERM` then policy-deadline `KILL` escalation, fail-closed exact-name lookup, ownership labels, and independently bounded mandatory cleanup.
- The Linux backend is covered by fake-runtime tests and an ignored physical NVIDIA isolation test. It is not wired into `remote-session connect` and therefore cannot execute production or paid jobs.
- BN-12 does not implement the Windows WSL2 backend, job submission, customer artifact download, result upload, arbitrary shell execution, production runtime verification, active-job remote cancellation discovery, paid execution, Kubernetes, or distributed workloads.
