...
- Production `remote-session connect` intentionally does not enable provider jobs. Disconnected Linux-native and Windows/WSL2 Docker/NVIDIA executor backends and the provider artifact byte plane now exist, but customer input ingestion, external object-storage signing/adapters, physical Windows GPU-isolation verification, verified runtime admission, remote active-job cancellation discovery, controlled activation, paid execution, multi-GPU jobs, and multi-provider jobs remain unimplemented. Scheduler leases and later metering/marketplace registries remain separate backend capabilities.
