...
- BN-12 does not implement automatic WSL2/Docker/driver setup, job submission, customer artifact download, result upload, arbitrary shell execution, production runtime verification, active-job remote cancellation discovery, paid execution, Kubernetes, or distributed workloads.
...
- `burd-agent` now contains a provider job orchestrator separated from its executor interface. It polls with the current authenticated session, revalidates complete bundles plus local provider/device/session/GPU bindings and expiries, accepts assignments, reports `provisioning`, `running`, and `uploading`, and terminates through `SubmitJobResult`. It executes only one assignment at a time and rejects bounded in-process replays.
