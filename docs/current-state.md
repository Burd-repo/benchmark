...
- Production `remote-session connect` intentionally does not enable provider jobs until the real container executor exists; the fake executor cannot create paid or production results. Container execution, byte-level artifact transfer, object-storage signed URLs, remote active-job cancellation discovery, runtime cleanup enforcement, paid execution, multi-GPU jobs, and multi-provider jobs remain unimplemented. Scheduler leases and later metering/marketplace registries remain separate backend capabilities.
