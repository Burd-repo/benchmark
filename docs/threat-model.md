...
| Lease or runtime authority changes after assignment | Credential issuance persists the exact `assignment_lease_id`. Acceptance requires that ID, locks the pair, checks offered status, server-side expiry and exact bindings, re-evaluates Runtime Admission, and only then atomically accepts exactly one lease. Current authority loss requeues the job and invalidates its credential. |
| Delayed acknowledgement from an older assignment | The requested `lease_id` must equal the persisted assignment binding. A stale, missing, or mismatched acknowledgement returns `409` without mutating the current job, lease, or credential, and no latest-lease fallback exists. |
| Cancelled job continues consuming provider resources | After acceptance, the Agent polls an authenticated execution-control endpoint bound to the exact job, lease, provider, device and session. A cancel directive propagates through data-plane operations and the executor, then cleanup runs without a contradictory failed result. |
| Stale cancellation watcher affects a newer assignment | Each watcher is bound to one persisted `assignment_lease_id`, stale control requests fail closed for only that worker, and the watcher is stopped and joined before its assignment returns. |
| Agent update activates provider jobs unexpectedly | Provider jobs remain disabled by default. Canary startup requires an explicit CLI mode, `--proofs`, native Linux, exact digest-pinned workload/helper images, and successful read-only Docker/NVIDIA/image preflight. There is no environment-variable or permissive fallback activation. |
| Control Plane outage leaves a workload running indefinitely | Transport and server errors are retried only until the independent maximum Control Plane silence bound; exceeding it cancels local execution fail closed. Artifact HTTP DNS, connect, send, and receive phases are capped at 30 seconds under a separate 120-second whole-call ceiling. |
