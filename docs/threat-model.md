...
| Control Plane outage leaves a workload running indefinitely | Transport and server errors are retried only until the independent maximum Control Plane silence bound; exceeding it cancels local execution fail closed. No-progress artifact HTTP I/O remains subject to its separate bounded transport timeout. |
